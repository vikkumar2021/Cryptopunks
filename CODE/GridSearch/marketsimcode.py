...
def marketsim_df(
    ...
    #    ********************  PLOT   *****************************
    df_temp = pd.concat(
        [portvals_benchmark_norm, portvals_manual_norm, portvals_norm],
        keys=["portvals_benchmark", "portvals_manual", "portvals_opto"],
        axis=1,
    )
    aa = df_temp.plot(
        title="Benchmark vs Manual vs Optimization strategy Normalized portvals - IN SAMPLE",
        color=["b", "black", "r"],
        label="benchmark",
    )
    aa.set_xlabel("Date")
