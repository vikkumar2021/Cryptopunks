...
class ManualLearner(object):
    ...
    def testPolicy(
        ...
        inddf = pd.DataFrame(
            list(zip(buyind, sellind, holdingslist)),
            columns=["buyind", "sellind", "holdings"],
        )
        # display(inddf)
        ...
        gains = compute_portvals(
            df, start_val=sv, commission=self.commission, impact=self.impact
        )
        order = [
            (1000, prices["index"].loc[0], "BUY", symbol),
            (0, prices["index"].loc[len(prices) - 1], "SELL", symbol),
        ]
        dfbench = pd.DataFrame(order, columns=["Shares", "Date", "Order", "Symbol"])
        benchmark = compute_portvals(
            dfbench, start_val=sv, commission=self.commission, impact=self.impact
        )
        ...
        textfile.write(
            "Portfolio Value: " + str(gains.values[len(gains) - 1][0]) + "\n"
        )
        ...
        for s, g in zip(statname, benchmarkstats):
            textfile.write(s + ": " + str(g) + "\n")
        textfile.write(
            "Portfolio Value: " + str(benchmark.values[len(gains) - 1][0]) + "\n"
        )
