...
def get_sharpe_ratio(threshold, *params):
    ...
    for day in range (lookback+1, normed.shape[0]):
        ...
        if((sma.ix[day, syms[0]] <threshold[0]) and (bbp.ix[day, syms[0]] < threshold[1]) and (rsi.ix[day, syms[0]] < 100*threshold[1]) and (-1000<=holdings[syms[0]]<1000) ):
            if holdings[syms[0]] < 1000: #stock oversold but index is not oversold
                ...
                Order.append('BUY')                    
        elif((sma.ix[day, syms[0]]>threshold[2]) and (bbp.ix[day, syms[0]]>threshold[3]) and (rsi.ix[day, syms[0]]>100*threshold[3]) and (-1000<=holdings[syms[0]]<=1000) ): #stock overbought but index is not overbought
            if holdings[syms[0]] > -1000:
                ...
                Symbol.append(symbol_str)
                Order.append('SELL')
        elif((sma.ix[day, syms[0]]>=(threshold[2])) and (sma.ix[day-1, syms[0]]<(threshold[2])) and (-1000<holdings[syms[0]]<=1000)and (-1000<=holdings[syms[0]]<=1000) ): # crossed SMA upwards and hold long
            #print ('3333 ')
            if holdings[syms[0]]==0:
                holdings[syms[0]] = holdings[syms[0]] - 1000
                Shares.append(1000)
            ...
            Symbol.append(symbol_str)
            Order.append('SELL')
        elif((sma.ix[day, syms[0]]<=(threshold[0])) and (sma.ix[day-1, syms[0]]>(threshold[0])) and (-1000<=holdings[syms[0]]<1000)): # crossed SMA downwards and hold short
            #print '4444 '
            if holdings[syms[0]]==0:
...
class StrategyLearner(object):
    ...
    def addEvidence(self, symbol = "IBM", \
        ...
        rranges_old = [slice(0.54, 1.1, 0.2),       # BUY SMA 0.44 0.64 0.84 1.04    
                   slice(0.2, 0.41, 0.2),     # BUY BBP-RSI   0.2 0.4
                   slice(0.55, 1.11, 0.3),          # SELL SMA  0.55 0.85 1.15
                   slice(0.58, 0.81, 0.2),]    # SELL BBP-RSI  0.58, 0.78

        rranges = [slice(0.6, 1.01, 0.2),       #  SMA  RATIO
                   slice(-0.2, 1.21, 0.2),      # BUY 
                   slice(1.0, 1.61, 0.2),     # SELL SMA
                   slice(-0.2, 1.21, 0.2),]    #  SELL BBP-RSI 

    #    print rranges
    #    print len(rranges)
    #    print len(initial_threshold)
        threshold_brute = spo.brute(get_sharpe_ratio, rranges, args = params, full_output=True, finish=None )
    ...
    def testPolicy(self, symbol = "JPM", sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31), sv = 100000):    
        ...
        print ('********************* Test Policy Computing SMA  ************************' )  
        sma_ratio = prices.copy()
        sma = normed.cumsum()
        sma.values[lookback:,:] = (sma.values[lookback:,:] - sma.values[:-lookback,:]) / lookback
        sma.ix[:lookback, :] = np.nan
        ...
        print ('********************* Test Policy Computing Bollinger Bands  ************************')
        bbp = normed.copy()
        bottom_band = normed.copy()
        top_band = normed.copy()
        rolling_std = normed.rolling(window=lookback, min_periods=lookback).std()
        ...
        print ('********************* Test Policy Computing RSI  ************************' )
        rsi = normed.copy()
        for day in range(normed.shape[0]):
            for sym in symbol:
                up_gain = 0
        ...
        for day in range (lookback+1, normed.shape[0]):
            ...
            if((sma.ix[day, sym] < self.threshold[0]) and (bbp.ix[day, sym] < self.threshold[1]) and (rsi.ix[day, sym] < 100*self.threshold[1])):
            ...
            elif((sma.ix[day, sym]>self.threshold[2]) and (bbp.ix[day, sym]>self.threshold[3]) and (rsi.ix[day, sym]>100*self.threshold[3]) ): #stock overbought but index is not overbought
            ...
            elif((sma.ix[day, sym]>=(self.threshold[2])) and (sma.ix[day-1, sym]<(self.threshold[2])) and (-1000 < holdings[sym]<=1000) ): # crossed SMA upwards and hold long
                if holdings[sym]==0:
                    holdings[sym] = holdings[sym] - 1000
                    Shares.append(1000)
                else:
            ...
            elif((sma.ix[day, sym]<=(self.threshold[0])) and (sma.ix[day-1, sym]>(self.threshold[0])) and ( -1000 <= holdings[sym] < 1000)): # crossed SMA downwards and hold short
                if holdings[sym]==0:
                    holdings[sym] = holdings[sym] + 1000
                    Shares.append(1000)
                else:
    ...
    def testPolicy_old(self, symbol = "JPM", sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31), sv = 100000):    
        ...
        for day in range (lookback+1, normed.shape[0]):
            ...
            if((sma.ix[day, sym] < self.threshold[0]) and (bbp.ix[day, sym] < self.threshold[1]) and (rsi.ix[day, sym] < 100*self.threshold[1])):
                ...
                if holdings[sym] > -1000:
                    ...
                    print ('RULE 2, SELL at ', normed.index[day], holdings[sym])
            elif((sma.ix[day, sym]>=(self.threshold[2]-0.05)) and (sma.ix[day-1, sym]<(self.threshold[2]-0.05)) and (-1000 < holdings[sym]<=1000) ): # crossed SMA upwards and hold long
                if holdings[sym]==0:
                    holdings[sym] = holdings[sym] - 1000
                    Shares.append(1000)
                else:
                ...
                print ('RULE 3, SELL at ', normed.index[day], holdings[sym])
            elif((sma.ix[day, sym]<=(self.threshold[0]+0.05)) and (sma.ix[day-1, sym]>(self.threshold[0]+0.05)) and ( -1000 <= holdings[sym] < 1000)): # crossed SMA downwards and hold short
                if holdings[sym]==0:
                    holdings[sym] = holdings[sym] + 1000
                    Shares.append(1000)
                else:
