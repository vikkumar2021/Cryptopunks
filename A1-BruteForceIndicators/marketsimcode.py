    ...
    for day in range (lookback+1, normed.shape[0]):
        ...
        if((sma.ix[day, syms[0]] <threshold[0]) and (bbp.ix[day, syms[0]] < threshold[1]) and (rsi.ix[day, syms[0]] < threshold[2]) and (-1000<=holdings[syms[0]]<1000) ):
            ...
            if holdings[syms[0]] > -1000:
                ...
                Symbol.append(symbol)
                Order.append('SELL')
        elif((sma.ix[day, syms[0]]>=threshold[6]) and (sma.ix[day-1, syms[0]]<threshold[6]) and (-1000<holdings[syms[0]]<=1000)and (-1000<=holdings[syms[0]]<=1000) ): # crossed SMA upwards and hold long
            if holdings[syms[0]]==0:
                holdings[syms[0]] = holdings[syms[0]] - 1000
                Shares.append(1000)
                print ('3333 ')
...
def marketsim_df(order = [], start_val = 10000, commission=9.95, impact=0.005, sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31)):
