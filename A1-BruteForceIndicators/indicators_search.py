...
def benchmark(order = [], start_val = 1000000, commission=9.95, impact=0.005, sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31)):
...
def get_manual(symbol = "JPM", start_val = 10000, commission=9.95, impact=0.005, sd = dt.datetime(2010, 1, 1), ed = dt.datetime(2011, 12, 31)):
    ...
    for day in range (lookback+1, normed.shape[0]):
        ...
        if((sma.ix[day, syms[0]] <threshold[0]) and (bbp.ix[day, syms[0]] < threshold[1]) and (rsi.ix[day, syms[0]] < threshold[2]) and (-1000<=holdings[syms[0]]<1000) ):
            if holdings[syms[0]] < 1000: #stock oversold but index is not oversold
                ...
                Order.append('BUY')                    
        elif((sma.ix[day, syms[0]]>threshold[3]) and (bbp.ix[day, syms[0]]>threshold[4]) and (rsi.ix[day, syms[0]]>threshold[5]) and (-1000<=holdings[syms[0]]<=1000) ): #stock overbought but index is not overbought
            if holdings[syms[0]] > -1000:
                ...
                Symbol.append(symbol)
                Order.append('SELL')
        elif((sma.ix[day, syms[0]]>=threshold[6]) and (sma.ix[day-1, syms[0]]<threshold[6]) and (-1000<=holdings[syms[0]]<=1000) ): # crossed SMA upwards and hold long
            if holdings[syms[0]]==0:
                holdings[syms[0]] = holdings[syms[0]] - 1000
                Shares.append(1000)
                print ('3333 ')
            ...
            Symbol.append(symbol)
            Order.append('SELL')
        elif((sma.ix[day, syms[0]]<=threshold[7]) and (sma.ix[day-1, syms[0]]>threshold[7]) and (-1000<=holdings[syms[0]]<1000)): # crossed SMA downwards and hold short
            if holdings[syms[0]]==0:
                holdings[syms[0]] = holdings[syms[0]] + 1000
                Shares.append(1000)
                print ('4444 ')
...
#    end_date = dt.datetime(2011,12,31)
    ...
    portvals_benchmark = benchmark(order = df_benchmark, start_val = start_val, commission=0.0, impact=0.0, sd = sd, ed = ed)
    ...
    portvals_manual = get_manual(symbol = syms[0], start_val = start_val, commission=0, impact=0,  sd = sd, ed = ed)
    portvals_manual_norm = portvals_manual/portvals_manual.values[0]
    
    cum_ret, avg_daily_ret, std_daily_ret, sharpe_ratio = assess_porfolio(port_val = portvals_benchmark)    
    print ('\n\n**********  BENCHMARK  **********')
    print ("last_day_portval = {}".format(portvals_benchmark[-1])) #last value or portval
    print ("sharpe_ratio = {}".format(sharpe_ratio))
    print ("Cumulative return = {}".format(cum_ret))

    cum_ret, avg_daily_ret, std_daily_ret, sharpe_ratio = assess_porfolio(port_val = portvals_manual)    
    print ('\n**********  MANUAL STRATEGY  **********')
    ...
    df_temp = pd.concat([portvals_benchmark, portvals], keys=['portvals_benchmark', 'portvals_opto'], axis=1)
    aa = df_temp.plot(title='Benchmark vs Optimization strategy portvals - IN SAMPLE', color = ['b', 'r'],label='benchmark')  
    aa.set_xlabel("Date")
...
def run_logic():
    ...
    portvals_opto = marketsim(order = df_trades, start_val = sv, commission=commission, impact=impact, sd = start_date_i, ed = end_date_i)
